...
class MSFAction(SessionAction):
    ...
    def get_local_source_interface(self, local_session: Session, remote_address: IPv4Address, environment: Environment) -> (Session, Interface):
        # discovers the local session and interface from routing through existing sessions
        remote_subnet = environment.get_subnet_containing_ip_address(remote_address)
        ...
        for interface in environment.hosts[local_session.host].interfaces:
            # find if remote address is in the sessions subnet
            if remote_address in interface.subnet:
                return local_session, interface

            # find if the remote address is in a routable subnet
            if interface.name != 'lo' and environment.subnets[interface.subnet].name in remote_subnet.nacls:
                return local_session, interface
