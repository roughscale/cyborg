...
class GreenConnection(Action):
    ...
    def sim_execute(self, environment: Environment) -> Observation:
        ...
        if type(environment.sessions[self.agent][self.session]) is GreenAbstractSession and self.ip_address in environment.sessions[self.agent][self.session].ports:
            ports = environment.sessions[self.agent][self.session].ports[self.ip_address]
            ...
            if self.ip_address != lo and obs.data['success'] == True:
                ...
                environment.sessions[self.agent][self.session].addos(hostname, os)
        else:
