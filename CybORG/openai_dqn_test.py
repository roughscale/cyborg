...
final_epsilon = 0.1  # explore/exploit factor
...
for ep in range(num_episodes):
  ...
  while not done and step <= step_limit_reached:
    ...
    # update Q function
    obs_hash, next_obs_hash, loss, mean_v = agent.train(Results(observation=state,next_observation=next_state,reward=reward,done=done))
