...
epsilon = 0.1  # explore/exploit factor
...
state=env.env.observation_change(copy.deepcopy(unwrapped_state))
state_space=env.observation_space
action_space=env.action_space

config = {"gamma": gamma, "epsilon": epsilon, "alpha": alpha }
# initialise agent learning
agent.agent.initialise(state_space,action_space,config)
...
for ep in range(num_episodes):
  ...
  while not done and step <= step_limit_reached:
    ...
    action_num=agent.get_action(state, action_space)
    ...
    next_state = env.observation_change(flattened_state)
    # update Q function
    obs_hash, next_obs_hash = agent.train(Results(observation=state,next_observation=next_state,reward=reward,done=done))
    print("Reward: {0}".format(reward))
    print("Done: {0}".format(done))
    episodic_states.add(obs_hash)
    episodic_states.add(next_obs_hash)
    print()
    last_state = state
    state = next_state
    step += 1
